...
(function () {
  ...
  // ---- Cart API (de-duplicated) ----
  window.Cart = {
    get() { return readLS(); },

    // Return the internal items array (safe helper for code expecting Cart.items())
    items() {
      const c = readLS();
      return Array.isArray(c.items) ? c.items : [];
    },

    // add(sku, qty, variant) — if same sku+variant exists, increase qty; else push new line
    add(sku, qty, variant) {
      const c = readLS();
      const vk = variantKey(variant);
    ...
    // Map to GA4 items using window.CATALOG for metadata
    ga4Items() {
      const c = readLS();
      return c.items.map(line => {
        const p = window.CATALOG ? window.CATALOG[line.sku] : null;
        return {
          item_id: p?.sku || line.sku,
          item_name: p?.name || line.sku,
    ...
    // Compute total using GA4 items
    total() {
      return this.ga4Items().reduce((sum, i) => sum + (Number(i.price)||0) * (Number(i.quantity)||1), 0);
    }
  };
  ...
  function bindAddToCartFromGrid() {
    document.querySelectorAll('.add-to-cart').forEach(btn => {
      btn.addEventListener('click', function () {
        ...
        // Add to cart (once)
        window.Cart.add(sku, 1, null);

        const qty = 1;
        window.pushEvent('add_to_cart', {
          currency: CURRENCY_DEFAULT,
          value: Number(card.dataset.price || '0') * qty,
          items: [{
            item_id: card.dataset.sku,
            item_name: card.dataset.name,
            item_category: card.dataset.category,
        ...
        alert(`${card.dataset.name || 'Item'} added to cart.`);
      });
    });
  }
  ...
  function bindCheckoutButtons() {
    ...
    if (beginBtn) {
      beginBtn.addEventListener('click', function () {
        window.pushEvent('begin_checkout', {
          currency: CURRENCY_DEFAULT,
          value: window.Cart.total(),
